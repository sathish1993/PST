...
Data * vectorRead(Vector * array, int index){
	...
	if(index >= array->max_size){
		...
		if(check==1){
			...
			outboundtemp_array = malloc(sizeof(Data) * array->max_size);
			memset(outboundtemp_array,-1,sizeof(Data)*array->max_size);
                	memcpy(outboundtemp_array,array->data,sizeof(Data)*array->current_size);
			free(array->data);
			array->data = NULL;
			array->data = outboundtemp_array;
			return &array->data[index];
		}
	}
}
